--[[
	Dreame Duels – Utility Suite v1.4 (Hook‑Free Instant Reset)
	All core functions preserved; stable and crash‑free.
--]]
...
-- ===== INSTANT RESET – HOOK‑FREE SCANNING =====
local RESET_GUID = "f888ee6e-c86d-46e1-93d7-0639d6635d42"
local resetRemoteCandidates = {}

local function refreshResetCandidates()
	resetRemoteCandidates = {}
	for _, d in ipairs(game:GetDescendants()) do
		if d:IsA("RemoteEvent") and d.Name:sub(1, 3) == "RE/" then
			table.insert(resetRemoteCandidates, d)
		end
	end
	-- fallback: any RemoteEvent with reset/respawn/revive in name
	for _, d in ipairs(game:GetDescendants()) do
		if d:IsA("RemoteEvent") and not d.Name:sub(1, 3) == "RE/" then
...
local function cursedInstaReset()
	local remote = nil
	for _, cand in ipairs(resetRemoteCandidates) do
	...
	task.spawn(function()
		local attempts = 0
		while attempts < 60 and not resetDetected do
			pcall(function() remote:FireServer(RESET_GUID, LP, "balloon") end)
			pcall(function() remote:FireServer(LP, RESET_GUID, "balloon") end) -- alternate order
			attempts = attempts + 2
			task.wait(0.05)
		end
		...
		pcall(function()
			...
			elseif alPhase == 2 then
				...
				if (tgt - hrp.Position).Magnitude < 1 then
					...
					alPhase = 1; if autoLeftSetVisual then autoLeftSetVisual(false) end; if refreshMobileButtons then refreshMobileButtons() end; return
				end
				local d = AP_L2 - hrp.Position; local mv = Vector3.new(d.X, 0, d.Z).Unit
				hum:Move(mv, false); hrp.AssemblyLinearVelocity = Vector3.new(mv.X * spd, hrp.AssemblyLinearVelocity.Y, mv.Z * spd)
			end
		...
		pcall(function()
			...
			elseif arPhase == 2 then
				...
				if (tgt - hrp.Position).Magnitude < 1 then
					...
					arPhase = 1; if autoRightSetVisual then autoRightSetVisual(false) end; if refreshMobileButtons then refreshMobileButtons() end; return
				end
				local d = AP_R2 - hrp.Position; local mv = Vector3.new(d.X, 0, d.Z).Unit
				hum:Move(mv, false); hrp.AssemblyLinearVelocity = Vector3.new(mv.X * spd, hrp.AssemblyLinearVelocity.Y, mv.Z * spd)
			end
...
local function setupSpeedIndicator(char)
	...
	pcall(function()
		...
		discordLabel.Text = "Dreame Duels"
		discordLabel.TextColor3 = Color3.fromRGB(212, 175, 55)
...
local function saveConfig()
	pcall(function()
		...
		if writefile then writefile("dreame_config.json", HS:JSONEncode(cfg)) end
	end)
end
...
-- ====== BUILD GUI with delay and rebrand ======
local function buildGui()
	local BG    = Color3.fromRGB(8, 8, 10)
	local BG2   = Color3.fromRGB(14, 14, 18)
	local CARD  = Color3.fromRGB(20, 20, 26)
	local HOV   = Color3.fromRGB(30, 30, 38)
	local GOLD  = Color3.fromRGB(212, 175, 55)
	local GOLDDIM = Color3.fromRGB(130, 100, 30)
	local STROKE = Color3.fromRGB(40, 40, 50)
	local W     = Color3.fromRGB(235, 235, 235)
	local DIM   = Color3.fromRGB(100, 100, 110)
	local INP   = Color3.fromRGB(16, 16, 22)
	local OFF   = Color3.fromRGB(30, 30, 38)

	local old = game:GetService("CoreGui"):FindFirstChild("DreameDuels")
	if old then old:Destroy() end
	local pg = LP:FindFirstChild("PlayerGui")
	if pg then local o = pg:FindFirstChild("DreameDuels"); if o then o:Destroy() end end

	local gui = Instance.new("ScreenGui")
	gui.Name = "DreameDuels"; gui.ResetOnSpawn = false; gui.DisplayOrder = 10; gui.IgnoreGuiInset = true
	...
	ttl.BackgroundTransparency = 1; ttl.Text = "DREAME DUELS"
	ttl.TextColor3 = GOLD; ttl.Font = Enum.Font.GothamBlack; ttl.TextSize = 14
	...
	closeBtn.Text = "-"; closeBtn.TextColor3 = GOLDDIM; closeBtn.Font = Enum.Font.GothamBold; closeBtn.TextSize = 22
	Instance.new("UICorner", closeBtn).CornerRadius = UDim.new(0, 6)
	closeBtn.MouseEnter:Connect(function() TS:Create(closeBtn, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(30, 20, 10), TextColor3 = GOLD }):Play() end)
	closeBtn.MouseLeave:Connect(function() TS:Create(closeBtn, TweenInfo.new(0.1), { BackgroundColor3 = BG2, TextColor3 = GOLDDIM }):Play() end)

	local miniBtn = Instance.new("TextButton", gui)
	miniBtn.Size = UDim2.new(0, 120, 0, 28); miniBtn.Position = UDim2.new(0, 26, 0, 26)
	miniBtn.BackgroundColor3 = BG2; miniBtn.BorderSizePixel = 0
	miniBtn.Text = "DUELS"
	miniBtn.TextColor3 = GOLD; miniBtn.Font = Enum.Font.GothamBold; miniBtn.TextSize = 11
	...
	mobileButtonsFrame.Size = UDim2.new(0, 184, 0, 332); mobileButtonsFrame.Position = UDim2.new(1, -204, 0.5, -166)
	mobileButtonsFrame.BackgroundTransparency = 1; mobileButtonsFrame.BorderSizePixel = 0
	mobileButtonsFrame.Active = true; mobileButtonsFrame.Visible = mobileButtonsVisible; mobileButtonsFrame.ZIndex = 25
	drag(mobileButtonsFrame)

	local mobileRefs = {}
	local function setMobileButton(id, on)
		local ref = mobileRefs[id]; if not ref then return end
		TS:Create(ref.btn, TweenInfo.new(0.12), { BackgroundColor3 = on and Color3.fromRGB(60, 50, 15) or Color3.fromRGB(5, 5, 8), TextColor3 = on and GOLD or W }):Play()
		TS:Create(ref.stroke, TweenInfo.new(0.12), { Color = on and GOLD or Color3.fromRGB(20, 20, 28) }):Play()
	end
	...
	local function mkMobileButton(order, id, txt, cb)
		...
		btn.Size = UDim2.new(0, 86, 0, 74); btn.Position = UDim2.new(0, col * 98, 0, row * 86)
		btn.BackgroundColor3 = Color3.fromRGB(5, 5, 8); btn.BorderSizePixel = 0
		btn.Text = txt; btn.TextColor3 = W; btn.Font = Enum.Font.GothamBlack; btn.TextSize = 14
		btn.TextWrapped = true; btn.AutoButtonColor = false; btn.ZIndex = 26
		btn.Active = true
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 16)
		local stroke = Instance.new("UIStroke", btn); stroke.Color = Color3.fromRGB(20, 20, 28); stroke.Thickness = 1.6; stroke.Transparency = 0.15
		mobileRefs[id] = { btn = btn, stroke = stroke }
		drag(btn)
		btn.MouseEnter:Connect(function() if not mobileRefs[id].down then TS:Create(btn, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(15, 15, 22) }):Play() end end)
	...
	local function mkSect(txt)
		...
		l.BackgroundTransparency = 1; l.Text = txt:upper(); l.TextColor3 = GOLD
	...
	local function mkRow(h)
		...
		Instance.new("UIStroke", f).Color = Color3.fromRGB(30, 30, 38)
	...
	local function animPill(pill, dot, on)
		TS:Create(pill, TweenInfo.new(0.18, Enum.EasingStyle.Quad), { BackgroundColor3 = on and Color3.fromRGB(130, 100, 30) or OFF }):Play()
		TS:Create(dot, TweenInfo.new(0.18, Enum.EasingStyle.Back), {
			Position = on and UDim2.new(1, -16, 0.5, -6.5) or UDim2.new(0, 3, 0.5, -6.5),
			BackgroundColor3 = on and GOLD or DIM
		}):Play()
	end
	...
	local function mkBox(parent, default, w, xOff, cb)
		...
		tb.Focused:Connect(function() TS:Create(bs, TweenInfo.new(0.12), { Color = GOLDDIM }):Play() end)
	...
	progressFill.Size = UDim2.new(0, 0, 1, 0); progressFill.BackgroundColor3 = GOLD; progressFill.BorderSizePixel = 0
	...
	do
		...
		modeValLbl.BackgroundTransparency = 1; modeValLbl.Text = "Normal"; modeValLbl.TextColor3 = GOLD
...
-- Config loading (file renamed)
local _savedCfg = nil
local function loadConfigKeys()
	if not (isfile and isfile("dreame_config.json")) then return end
	local ok, cfg = pcall(function() return HS:JSONDecode(readfile("dreame_config.json")) end)
...
print("Dreame Duels loaded successfully")
